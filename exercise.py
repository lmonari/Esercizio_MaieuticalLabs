...
class PolynomialCreator:
    ...
    def __init__(self, n_terms_min, n_terms_max, coefficients_range, exponent_range, unknown):
        if n_terms_min > n_terms_max:
            raise ArithmeticError("Minimum terms number must be lower than maximum terms number.")
        if n_terms_min < 1 or type(n_terms_min) != int or type(n_terms_max) != int:
            raise IOError("Number of terms must be a positive nonzero integer")
        ...
        with tag('html'):
            with tag('body'):
                ...
                with tag('section'):
                    ...
                    with tag('p', id='ans_A', style='margin-left:40px'):
                        text(self.answers[AnswersID.A])
                    with tag('p', id='ans_B', style='margin-left:40px'):
                        text(self.answers[AnswersID.B])
                    with tag('p', id='ans_C', style='margin-left:40px'):
                        text(self.answers[AnswersID.C])
                    with tag('p', id='ans_D', style='margin-left:40px'):
                        text(self.answers[AnswersID.D])
                    with tag('p', id='ans_E', style='margin-left:40px'):
                        text(self.answers[AnswersID.E])
                    with tag('p', id='ans_F', style='margin-left:40px'):
                        text(self.answers[AnswersID.F])
...
if __name__ == '__main__':
    ...
    unknown = UnknownVariable.x  # Incognita da utilizzare

    # Tool for creating polynomials
    poly_creator = PolynomialCreator(n_terms_min, n_terms_max, coefficients_range, exponent_range, unknown)
    ...
    answers = {
        AnswersID.A: poly_creator.new(n_terms=nA, order=PolynomialOrder.increasing),
        AnswersID.B: poly_creator.new(n_terms=nB, order=PolynomialOrder.increasing),
        AnswersID.C: poly_creator.new(n_terms=nC, order=PolynomialOrder.decreasing),
        AnswersID.D: poly_creator.new(),
        AnswersID.E: poly_creator.new(),
        AnswersID.F: poly_creator.new(),
    }
    ...
    # Create exercise
    ex = Exercise(delivery, answers, solution)
